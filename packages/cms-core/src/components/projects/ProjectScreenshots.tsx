...
interface ProjectScreenshotsProps {
  ...
  renderImage?: (props: { src: string; alt: string; onError: () => void }) => React.ReactNode;
}
...
function ScreenshotCard({
  ...
  filename: string;
  label: string;
  renderImage?: ProjectScreenshotsProps["renderImage"];
}) {
    ...
    <div className="rounded-xl border border-white/10 overflow-hidden hover:border-white/20 transition-colors">
      <div className="relative aspect-video bg-white/[0.02]">
        {!hasError && renderImage
          ? renderImage({ src, alt: label, onError: () => setHasError(true) })
          : !hasError
            ? <img src={src} alt={label} className="object-cover w-full h-full" onError={() => setHasError(true)} />
            : null}
        <div
