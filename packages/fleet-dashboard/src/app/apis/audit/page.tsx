...
export default function ApisAuditPage() {
      ...
      try {
        ...
        for (const site of json.sites) {
          ...
          for (const [provider, usage] of Object.entries(
            site.apiUsage.providers
          )) {
            selfReported.set(
              provider,
              (selfReported.get(provider) ?? 0) + usage.costUsd
            )
          }
        }
        ...
        const rows: AuditRow[] = [...allProviders]
          .map((provider) => {
            ...
            if (reported > 0 && invoiceUsd != null) {
              ...
              discrepancyPct =
                invoiceUsd > 0 ? (discrepancyUsd / invoiceUsd) * 100 : 0
              status =
                Math.abs(discrepancyPct) < 5 ? "matched" : "discrepancy"
            } else if (reported > 0) {
              status = "no-invoice"
  ...
  const totalSelfReported = auditRows.reduce(
    (s, r) => s + r.selfReportedUsd,
    0
  )
  const totalInvoiced = auditRows.reduce(
    (s, r) => s + (r.invoiceUsd ?? 0),
    0
  )
  ...
  return (
    <div className="space-y-8">
      <p className="text-sm text-zinc-500">
        Compare self-reported API usage against vendor invoices to find billing
        discrepancies.
      </p>
              ...
              {auditRows.map((row) => {
                ...
                return (
                  <tr key={row.provider} className="hover:bg-zinc-900/50">
                    ...
                    <td className="px-6 py-3 text-right text-zinc-400">
                      {row.invoiceUsd != null
                        ? `$${row.invoiceUsd.toFixed(2)}`
                        : "\u2014"}
                    </td>
                    <td
                      className={`px-6 py-3 text-right font-medium ${
                        row.discrepancyUsd != null &&
                        Math.abs(row.discrepancyUsd) > 1
                          ? "text-red-400"
                          : "text-zinc-500"
                      }`}
                    >
                      {row.discrepancyUsd != null ? (
                        <>
                          {row.discrepancyUsd >= 0 ? "+" : ""}$
                          {row.discrepancyUsd.toFixed(2)}
                          {row.discrepancyPct != null && (
                            <span className="ml-1 text-xs text-zinc-600">
                              ({row.discrepancyPct >= 0 ? "+" : ""}
                              {row.discrepancyPct.toFixed(1)}%)
                            </span>
                          )}
                        </>
                      ) : (
                    ...
                    <td className="px-6 py-3">
                      <span
                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset ${sc.badge}`}
                      >
                        {sc.label}
                      </span>
                    </td>
      ...
      <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-6">
        ...
        <ol className="list-inside list-decimal space-y-2 text-sm leading-relaxed text-zinc-400">
          <li>
            Each deployed site with the{" "}
            <code className="rounded bg-zinc-800 px-1 text-xs font-mono text-zinc-300">
              apiUsage
            </code>{" "}
            module tracks every outbound API call (tokens, cost, provider).
          </li>
          <li>
            The health endpoint self-reports aggregated usage to the Pandotic
            Hub.
          </li>
          <li>
            You add vendor invoice amounts to{" "}
            <code className="rounded bg-zinc-800 px-1 text-xs font-mono text-zinc-300">
              fleet.config.ts
            </code>{" "}
            (or they'll be auto-imported from your API key tool when ported).
          </li>
          <li>
