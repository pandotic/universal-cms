...
export async function GET(request: NextRequest) {
  try {
    ...
    const contentTable = params.get("contentTable") ?? undefined;

    const reviews = await listQAReviewsForContent(supabase, contentId, contentTable);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const review = await createQAReview(supabase, body);
