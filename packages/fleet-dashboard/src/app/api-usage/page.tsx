    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-500">
          Current month &middot; aggregated from {sites.length} site{sites.length !== 1 ? "s" : ""} reporting usage data
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-5">
          ...
          <p className="mt-1 text-2xl font-semibold text-white">{grandTotalRequests.toLocaleString()}</p>
        </div>
        <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-5">
          <p className="text-sm text-zinc-500">Total Cost</p>
          <p className="mt-1 text-2xl font-semibold text-white">${grandTotalCost.toFixed(2)}</p>
        </div>
        <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-5">
          <p className="text-sm text-zinc-500">Providers</p>
          <p className="mt-1 text-2xl font-semibold text-white">{allProviders.size}</p>
        </div>
      </div>
      ...
      {sortedProviders.length > 0 && (
        <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-6">
          <h2 className="mb-4 text-lg font-semibold text-white">Cost by Provider</h2>
          <div className="space-y-3">
            {sortedProviders.map(([provider, usage]) => {
              const pct = grandTotalCost > 0 ? (usage.costUsd / grandTotalCost) * 100 : 0;
                ...
                <div key={provider}>
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-zinc-400">
                      ${usage.costUsd.toFixed(2)} &middot; {usage.requests.toLocaleString()} calls
                    </span>
                  </div>
      ...
      {sites.length > 0 ? (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-800 bg-zinc-900/50">
                <th className="px-6 py-3 text-left font-medium text-zinc-400">Site</th>
                <th className="px-6 py-3 text-right font-medium text-zinc-400">Requests</th>
                <th className="px-6 py-3 text-right font-medium text-zinc-400">Cost</th>
                <th className="px-6 py-3 text-left font-medium text-zinc-400">Top Provider</th>
              </tr>
            </thead>
                  ...
                  return (
                    <tr key={site.siteUrl} className="hover:bg-zinc-900/50">
                      <td className="px-6 py-3">
                        ...
                        <div className="text-xs text-zinc-600">{site.environment}</div>
                      </td>
                      ...
                      <td className="px-6 py-3 capitalize text-zinc-400">
                        {topProvider ? `${topProvider[0]} ($${topProvider[1].costUsd.toFixed(2)})` : "—"}
                      </td>
                    </tr>
                  );
                })}
        ...
        <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-12 text-center">
          <h2 className="text-lg font-medium text-zinc-300">No API usage data yet</h2>
          <p className="mt-2 text-sm text-zinc-500">
            Enable the <code className="rounded bg-zinc-800 px-1.5 py-0.5 text-xs font-mono text-zinc-400">apiUsage</code> module
            in your deployed sites to start tracking API costs.
          </p>
        </div>
      )}
    </div>
