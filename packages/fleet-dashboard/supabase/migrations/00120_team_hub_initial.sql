...
-- ============================================================
CREATE TABLE IF NOT EXISTS standing_item_templates (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  name TEXT NOT NULL,
...
-- Add a unique constraint on name so the seed below is idempotent.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'standing_item_templates_name_key'
  ) THEN
    ALTER TABLE standing_item_templates
      ADD CONSTRAINT standing_item_templates_name_key UNIQUE (name);
  END IF;
END $$;
...
-- CHAIR ROTATION
-- ============================================================
CREATE TABLE IF NOT EXISTS chair_rotation (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- ============================================================
CREATE TABLE IF NOT EXISTS command_center_flags (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS idx_cc_flags_snapshot ON command_center_flags(snapshot_date DESC);
