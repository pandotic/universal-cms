...
CREATE INDEX IF NOT EXISTS idx_meeting_prep_meeting ON meeting_prep(meeting_id);

-- MEETING ISSUE ORDER (chair can reorder issues during IDS)
CREATE TABLE IF NOT EXISTS meeting_issue_order (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  meeting_id UUID NOT NULL REFERENCES meetings(id) ON DELETE CASCADE,
  issue_id UUID NOT NULL REFERENCES issues(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_meeting_issue_order ON meeting_issue_order(meeting_id, sort_position);
