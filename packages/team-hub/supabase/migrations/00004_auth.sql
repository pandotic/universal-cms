-- ============================================================
-- Pandotic Team Hub — Phase 4: Supabase Auth + tightened RLS
-- ============================================================
-- Adds auth.users -> public.users linkage via auth_user_id,
-- restricts sign-up to @pandotic.com, replaces blanket RLS
-- policies with identity-aware ones, and adds completed_by
-- on todos so we can tell who checked off which item.
-- ============================================================
...
DECLARE
  ...
  v_existing_id UUID;
BEGIN
  IF v_email NOT LIKE '%@pandotic.com' THEN
    RAISE EXCEPTION 'Only @pandotic.com email addresses may sign in';
  END IF;
