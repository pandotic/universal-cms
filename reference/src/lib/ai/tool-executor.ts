...
import type { PageType, PageStatus } from "@/lib/data/content-pages";
import {
  getAllContentPages,
  getContentPageById,
...
export async function executeTool(
  ...
  switch (name) {
    ...
    case "list_content_pages": {
      let pages = await getAllContentPages();
      if (input.status)
    ...
    case "get_content_page": {
      const page = input.id
        ? await getContentPageById(input.id as string)
        : input.slug
          ? await getContentPageBySlug(input.slug as string)
          : null;
      if (!page) {
        return {
          raw: null,
    ...
    case "update_content_page": {
      ...
      const page = await updateContentPage(id as string, updates);
      return {
        raw: page,
        display: {
          toolName: name,
    ...
    case "delete_content_page": {
      await deleteContentPage(input.id as string);
      return {
        raw: { deleted: true },
        display: {
          toolName: name,
    ...
    case "list_entities": {
      ...
      let query = sb.from("entities").select("id,name,slug,type,status,website");
    ...
    case "get_entity": {
      const sb = await getSupabaseAdmin();
      let query = sb.from("entities").select("*");
    ...
    case "get_site_setting": {
      const value = await getSetting(input.key as string);
      return {
        raw: value,
        display: {
          toolName: name,
    ...
    case "list_site_settings": {
      const settings = input.group_name
        ? await getSettingsByGroup(input.group_name as string)
